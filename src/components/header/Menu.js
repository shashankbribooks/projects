...
const Menu = (props) => {
	...
	const _logout = async() => {
		try {
			...
			dispatch({ type: 'UPDATE_BOOK', payload: {
				book: null
			}})
	...
	return (
		<Col className="text-end text-lg-start">
			...
			<Navbar.Offcanvas id="canvas-navbar-nav" placement="start" className="bg-light">
				...
				<Offcanvas.Body className="padding-horizontal-40">
					<Nav className="menu">
						{isMobile ? <>
							...
							<Link legacyBehavior  href="https://www.bribooks.com/account/myearnings">
								<a className="nav-link">
									<FontAwesomeIcon
							...
							{user?.id && <Nav.Item className="nav-link" onClick={_logout}>
								<FontAwesomeIcon
